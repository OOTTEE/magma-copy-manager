...
import { User, Zap, Share2, ChevronUp } from 'lucide-react';
...
export const ReportCard: React.FC<ReportCardProps> = ({ item, onCharge, enrichedUser }) => {
  ...
  return (
    <div className={`group bg-white dark:bg-[#1a1818] rounded-[2.5rem] border transition-all duration-500 shadow-lg hover:shadow-2xl overflow-hidden ${isExpanded ? 'border-indigo-500/30 ring-1 ring-indigo-500/20' : 'border-slate-200 dark:border-white/5'}`}>
      <div className="p-8">
        {/* Header */}
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            ...
            className={`p-3 rounded-2xl transition-all ${isExpanded ? 'bg-indigo-500 text-white' : 'bg-slate-50 dark:bg-white/5 text-slate-400 hover:text-indigo-500'}`}
          >
            {isExpanded ? <ChevronUp size={20} /> : <Share2 size={20} />}
          </button>
        </div>
