...
export const AdminDashboardView = () => {
    const { autoSyncError, fetchStatus } = useSystemStore();
        ...
        <div className="space-y-6">
            ...
            {autoSyncError && (
                <div 
                    ...
                    className="p-6 bg-red-500/10 border border-red-500/20 rounded-[2rem] flex items-center justify-between cursor-pointer group hover:bg-red-500/15 transition-all"
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-red-500 dark:text-red-400 tracking-tight">
                                Error en Sincronización Automática
                            </h3>
                            <p className="text-sm text-red-500/70 dark:text-red-400/60 font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-sm sm:max-w-md">
                                {autoSyncError}
                            </p>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                    <div 
                        key={i} 
                        className="h-48 rounded-[2rem] bg-indigo-500/5 border border-indigo-500/10 flex items-center justify-center"
                    >
