...
export const ReportTable: React.FC<ReportTableProps> = ({ 
  ...
  const { visibleData, hiddenData } = useMemo(() => {
    // 1. Usuarios con actividad: total > 0, ordenados por total desc
    const active = data
      ...
      <td className="px-8 py-5 text-center">
        {invoiceStatuses[item.id] ? (
          ...
          <button 
            ...
            className={`p-3 rounded-xl transition-all shadow-indigo-500/30 active:scale-90 ${
              item.total === 0 
                  ...
                  <div className="flex justify-center">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 dark:text-white/40 group-hover:text-slate-700 dark:group-hover:text-white transition-colors">
                        {showHidden ? 'Ocultar usuarios sin actividad' : `Mostrar ${hiddenData.length} usuarios sin actividad`}
                      </span>
