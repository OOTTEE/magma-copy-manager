...
export class AutoSyncService {
    ...
    async init() {
        ...
        if (enabled) {
            ...
            console.log(`[AutoSync] Scheduling job (${frequency}): ${cronExpr}`);
            this.scheduledTask = cron.schedule(cronExpr, () => {
                this.runJob({ id: 'system', role: 'admin' }).catch(err => {
                    console.error('[AutoSync] Cron Job Failed:', err);
                });
            });
        } else {
            console.log('[AutoSync] Scheduler disabled.');
        }
    }
    ...
    async runJob(requestingUser: { id: string; role: string }, triggerType: 'auto' | 'manual' = 'auto') {
        ...
        console.log(`[AutoSync] Starting job ${logId} at ${startTime}`);

        try {
            // Phase 1: Sync printer copies with 3 retries
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    console.log(`[AutoSync] Syncing printer (Attempt ${attempt}/3)...`);
                    ...
                    console.warn(`[AutoSync] Attempt ${attempt} failed:`, err.message);
            ...
            console.log(`[AutoSync] Job ${logId} finished with status: ${status}`);
        }
